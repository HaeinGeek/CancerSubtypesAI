...
def load_data():
    ...
    return mut_seq_df.merge(mut_encoding_df, on=['gene', 'mutation_str'], how='left')
...
def process_data(merged_df):
    ...
    merged_df_prot = merged_df.loc[(merged_df.type != 'WT') & 
                                   (merged_df.type != 'Silent_Missense') & 
                                   (merged_df.type != 'Silent_Nonsense') & 
                                   (merged_df.status_prot == 0)]

    wt_prot = merged_df_prot.drop_duplicates(subset=['isoform_id'])[['gene', 'isoform_id', 'wt_seq']]
    mut_prot = merged_df_prot.drop_duplicates(subset=['isoform_id', 'mutation_str'])[['gene', 'mutation_str', 'isoform_id', 'mut_seq']]
...
def main():
    setup_environment()
    merged_df = load_data()
    wt_prot, mut_prot = process_data(merged_df)
